...
_RULE_FILES_FOR_INDEX = ("ai-tells.md", "craft.md", "grammar.md", "style-rules.md")


def _build_rules_index() -> dict[str, str]:
    """Map rule-slug → full rule block text (heading + body up to next ###).

    Slugs not defined in a `slug · type · scope · severity` line (e.g. frame
    umbrella `f1`–`f7` and sub-pattern `f4/targhak-closure`) are not in this
    index — those live in SKILL.md and ride along whenever SKILL.md is in
    the bundle.
    """
    out: dict[str, str] = {}
    block_split_re = re.compile(r"^(### .+?)$", re.MULTILINE)
    slug_in_block_re = re.compile(
        r"^`([a-z0-9][a-z0-9/_-]*)`\s+·", re.MULTILINE
    )
    for fname in _RULE_FILES_FOR_INDEX:
        text = (KIEN_THAI_DIR / "references" / fname).read_text(encoding="utf-8")
        ...
        for i in range(1, len(parts), 2):
            ...
            full = heading + body
            m = slug_in_block_re.search(full)
            if m:
                out[m.group(1)] = full.rstrip() + "\n"
    return out


_AUDIT_SLUG_REF_RE = re.compile(
    r"`(f[1-7](?:/[a-z][a-z0-9-]*)?|[a-z][a-z0-9]*(?:-[a-z0-9]+)+)`"
)


def extract_cited_slugs(audit_text: str) -> list[str]:
    """Pull rule slugs cited in an audit response, in order, deduplicated."""
    seen: set[str] = set()
    out: list[str] = []
    for m in _AUDIT_SLUG_REF_RE.finditer(audit_text):
        slug = m.group(1)
        if slug not in seen:
            seen.add(slug)
            out.append(slug)
...
def kien_thai_slim_fix_bundle(register: str, cited_slugs: list[str]) -> str:
    """Build the minimal bundle for a fix pass.

    Includes: SKILL.md (workflow stripped, frames intact), the active
    register block, forbidden-phrases, and only the cited rules from
    ai-tells/craft/grammar/style-rules. Frame umbrella + sub-pattern slugs
    don't need explicit lookup — they live in SKILL.md.

    For unknown cited slugs (typos, new patterns the auditor invented),
    the slug name still ends up in the prompt's audit text — the model can
    ...
    # Active register block (always — fixer needs scope).
    reg_text = (KIEN_THAI_DIR / "references" / "register.md").read_text(encoding="utf-8")
    reg_text = _strip_default_meta(reg_text)
    reg_text = _scope_register_md(reg_text, register)
    parts.append(f"\n\n## reference: register.md\n\n{reg_text}")

    # Forbidden-phrases (small, used at audit pre-check; harmless on fix).
    fp = (KIEN_THAI_DIR / "references" / "forbidden-phrases.md").read_text(encoding="utf-8")
    parts.append(f"\n\n## reference: forbidden-phrases.md\n\n{fp}")

    # Cited rules only.
    index = _build_rules_index()
    ...
    for slug in cited_slugs:
        if slug in seen or slug not in index:
            continue
        seen.add(slug)
        cited_blocks.append(index[slug])
    if cited_blocks:
        parts.append("\n\n## cited rules\n\n" + "\n\n".join(cited_blocks))
